...
export function useFeedbackCheck(userId?: string) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      // Use async function within effect but don't block the UI
      (async () => {
        try {
          ...
          if (!devWaitlist) {
            console.log('📋 No dev-waitlist found in storage - user never joined any events');
            return;
          }

          const waitlistEvents = devWaitlist;
          ...
          const feedbackRecord = await getFeedbackRecord(userId, matchedTimeSlot, matchedEventDate);
          ...
          console.error('Error checking feedback requirements from storage:', e);
        }
      })();
    }
